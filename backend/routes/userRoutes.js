...
const giveSalary = async (user) => {
  try {
    ...
    while (sponser) {
      sponser = await User.findById(sponser);
      if (!sponser) {
        break;
      }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (user && user.imgStatus == "progress") {
      ...
      res.status(401);
      throw new Error(
        "Some error occured. Make sure you are not verified this user before!"
      );
    }
  })
);
