...
function VerifyUsersComponent() {
  ...
  const { loading, data, error } = useSelector((state) => state.getUserReducer);
  const { data: verifiedData, error: verifiedError } = useSelector(
    (state) => state.verifyUserReducer
  );
  const { data: rejectedData, error: rejectedError } = useSelector(
    (state) => state.getRejectedReducer
  );
